...
export default async function FetchArabicMovies(): Promise<ArabicMoviesItem[]> {
  ...
  return await HandleRequests(
    "/discover/movie",
    `&language=ar&with_original_language=ar&with_origin_country=EG&sort_by=popularity.desc&first_air_date.gte=${formattedDate}`,
    "فيلم"
  );
}
