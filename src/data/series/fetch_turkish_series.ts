...
export default async function FetchDiscoverTurkishSeries(): Promise<TurkishSeriesItem[]> {
  ...
  return await HandleRequests(
    "/discover/tv",
    `&with_original_language=tr&sort_by=popularity.desc&first_air_date.gte=${formattedDate}`,
    "مسلسل"
  );
}