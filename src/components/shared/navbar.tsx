...
// Navbar Links
const navLinks = [
  { label: "الرئيسية", icon: <Home className={iconSize} /> },
  { label: "الأحدث", icon: <Clock className={iconSize} /> },
  { label: "الأفلام", icon: <Film className={iconSize} /> },
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Desktop Navbar */}
      <header onClick={handleClickInside}
        className="fixed top-3 left-1/2 -translate-x-1/2 z-50 hidden md:flex w-[95%] max-w-7xl h-16 px-6 justify-between items-center rounded-full border-2 backdrop-blur-xl shadow-md bg-black/20" >
        ...
        {/* logo */}
        <Image src="/images/moovy.webp" alt="logo" width={80} height={80} priority unoptimized className="cursor-pointer" />
      </header>

      {/* Mobile Top Navbar */}
      <header className="md:hidden fixed top-0 left-0 w-full h-14 px-4 flex items-center justify-between z-50 ">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Button variant="default" className="text-xs px-2">تسجيل دخول</Button>
        </div>
      </header>
